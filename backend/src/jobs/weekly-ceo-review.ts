...
export async function runWeeklyCeoReviewJob(deps: AppDeps): Promise<JobResult> {
  ...
  try {
    ...
    const [customersResult, agentsResult, funnelSummary, npsStats] = await Promise.all([
    ...
    await deps.clients.email.send({
      ...
      html,
      text: `MRR: ${formatUsd(mrrCents)} | Active customers: ${activeCount} | Churn: ${churnRate30d.toFixed(1)}% | NPS: ${npsStats.nps_score ?? "n/a"}`
    });
